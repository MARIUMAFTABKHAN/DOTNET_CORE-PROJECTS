...
namespace CDSN
{
    public partial class HeadendViewershipreport : System.Web.UI.Page
    {
        ...
        protected void BtnTerritory_Click(object sender, ImageClickEventArgs e)
        {
            ...
            var legendList = db.usp_GetHeadendviewershipByTerritory(110000001, selectedTerritories).ToList();

            int selectedTotal = legendList.Sum(x => x.Val ?? 0);

            int remainingViewers = Convert.ToInt32(db.usp_GetHeadendViewership_TerritoryLegends(110000001, selectedTerritories).SingleOrDefault());

            GridViewTerritory.DataSource = legendList;
            GridViewTerritory.DataBind();

            // Prepare chart data
            var labels = legendList.Select(x => x.Territory).ToList();
            var values = legendList.Select(x => x.Val ?? 0).ToList();

...
{
    /// <summary>
    /// Summary description for SaveDocHandler
    /// </summary>
    public class SaveDocHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                var clientContactId = HttpContext.Current.Request.Params["msg"];
                //foreach (var Txt in clientContactId)
                //{
                //    string txtm = Txt.ToString();
                //}
                HttpPostedFile postedFile = context.Request.Files["file"];
                string ext2 = System.IO.Path.GetExtension(postedFile.FileName);

                string strJson = context.Request.QueryString[0];
                Employee objUsr = Deserialize<Employee>(strJson);
                // HttpPostedFile postedFile = (File) file;
                using (DBScanEntities db = new DBScanEntities())
                {
                    if (objUsr != null)
                    {
                        string userid = objUsr.userid;
                        string ronumber = objUsr.ronumber.Substring(17);
